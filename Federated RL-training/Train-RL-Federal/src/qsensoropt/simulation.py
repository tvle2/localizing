...
from tensorflow import zeros, ones, reduce_mean, \
    ...
    gather,device,switch_case,floor,dtypes,compat,config
...
#         return exp_decay * noise_less + (1.0 - exp_decay) / 2.0
    ...
    def Nmodel(
        self,
        n_shots,
        outcomes,           # tf.Tensor, shape (N, n_shots), values in {-1, +1}
        evolution_time,     # scalar float or tf scalar (τ)
        omega,              # tf.Tensor of shape (N, 1, 1); here it is (ω τ)
        F0: float = 0.88,
        F1: float = 0.95,
    ):
        """BNN-side likelihood with readout fidelity (F0, F1) and fixed T2=1500.

        NOTE:
        - 'omega' passed here is already (ω τ), so DO NOT multiply by τ again.
        - This returns p(observed outcomes) elementwise, matching the caller's
          convention: if 'outcomes' is all +1, it yields p(+1); if it's sampled
          outcomes, it yields p(sampled outcome).
        """
        # Fixed T2 for this experiment
        invT2 = 1.0 / 1500.0

        # Dephasing factor (scalar tf tensor)
        E = exp(-evolution_time * invT2)

        # Compute cos(ω τ) and broadcast to (N, n_shots)
        N = shape(outcomes)[0]
        c = cos(omega)                       # (N, 1, 1)
        c = reshape(c, (N, 1))               # (N, 1)
        c = c * ones((N, n_shots), dtype="float32")  # (N, n_shots)

        # Ideal probabilities (no readout error)
        p_true_plus  = 0.5 * (1.0 - E * c)   # (N, n_shots)
        p_true_minus = 1.0 - p_true_plus

        # Readout confusion matrix to match observed outcomes μ ∈ {−1, +1}
        mu = outcomes                         # (N, n_shots), values ±1
        mask_plus  = (mu + 1.0) * 0.5
        mask_minus = 1.0 - mask_plus

        p_obs_plus  = F1 * p_true_plus  + (1.0 - F0) * p_true_minus
        p_obs_minus = F0 * p_true_minus + (1.0 - F1) * p_true_plus
        prob = mask_plus * p_obs_plus + mask_minus * p_obs_minus  # (N, n_shots)
    ...
    # With fidelity
    def NNperform_measurement(
        self,
        n_shots,
        controls,      # scalar τ
        parameters,    # tf.Tensor (N, 1, 1) holding (ω τ)
        rangen: Generator = Generator.from_seed(0xDEADD0D0),
    ):
        """Sample BNN 'shots' given τ and (ω τ), using fidelity-aware likelihood."""
        # Probability of μ=+1 for each trial
        list_plus = ones((len(parameters), n_shots), dtype="float32")
        prob_plus = self.Nmodel(
            n_shots, list_plus, controls, parameters, F0=0.88, F1=0.95
        )

        # Draw outcomes μ ∈ {−1, +1} with Bernoulli(prob_plus)
        seed = self.get_seed(rangen)
        draws = stateless_uniform((len(parameters), n_shots), seed, dtype="float32")
        outcomes = 2 * cast(draws < stop_gradient(prob_plus), dtype="int8") - 1
        outcomes = cast(outcomes, dtype="float32", name="outcomes")

        # Probability of the actually observed outcomes (for log-likelihood)
        prob_outcomes = self.Nmodel(
            n_shots, outcomes, controls, parameters, F0=0.88, F1=0.95
        )
        log_prob = cast(log(prob_outcomes), dtype="float32", name="log_prob")
        return outcomes, log_prob
...
`
    ...
    def execute(
        ...
        **Achtung!** Ideally the user never needs to use
        ...
        print(idxN,'paTruees', true_values)
        if pars.permutation_invariant:
        ...
        for u in self.pf.phys_model.params:
            u.bounds = Nois[idxN]
        ...
        print(particles,'2-particles',particles.shape)
