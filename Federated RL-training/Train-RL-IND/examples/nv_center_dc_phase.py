...
def static_field_estimation(
    ...
    omega_bounds: Tuple[float, float] = (9*2,(9+1)*2),#(0.0,20)
    ):
...
def main():
    ...
    static_field_estimation(
        args, 1024, max_res=int(10000), learning_rate=1e-3, res='time',#max_res=int(2796)
        log_loss=True, cumulative_loss=True,
        invT2=1/96,
        )

    # file_path = args.data_dir + '/nv_center_time_invT2_0.0104_lr_0.001_batchsize_1024_num_steps_2796_max_resources_2796.00_ll_True_cl_True_ext.csv'
    # import pandas as pd
    # # Read the CSV file
    # data_df_loaded = pd.read_csv(file_path)
    #
    # # Display the first few rows
    # print(data_df_loaded.tail(-100))
