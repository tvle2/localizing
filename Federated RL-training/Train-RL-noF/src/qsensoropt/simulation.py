...
from tensorflow import zeros, ones, reduce_mean, \
    ...
    gather,device,switch_case,floor,dtypes,compat
...
`
    ...
    def execute(
        ...
        **Achtung!** Ideally the user never needs to use
        ...
        numN = 10
        Nois = [(2.5*i, 2.5*(i+1)) for i in range(numN)]#0.5+0.02*i

        def get_network_output(i):
            return lambda: Nois[i]

        #print([u.bounds  for u in self.phys_model.params])
        # for u in self.phys_model.params:
        #     u.bounds = Nois[idxN]
            #print(u.bounds,Nois[idxN])
        #switch_case(idxN, branch_fns={i: get_network_output(i) for i in range(len(Nois))})
        #print([u.bounds  for u in self.phys_model.params])
