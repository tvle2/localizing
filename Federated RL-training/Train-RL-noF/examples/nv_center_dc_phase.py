...
from tensorflow import  concat, Tensor,float32,config,switch_case,constant
...
def parse_args():
    ...
    parser.add_argument("--trained-models-dir", type=str,
                        default="./nv_center_dc_phase25/trained_models")
    parser.add_argument("--data-dir", type=str,
                        default="./nv_center_dc_phase25/data")
...
def static_field_estimation(
    ...
    omega_bounds: Tuple[float, float] = (0.0,2.5),#(0.0, 2*3.14*25),
    ):
    ...
    def control_nn(idxN:int,input_tensor: Tensor):
        ...
        def get_network_output(i):
            tau = prefact * abs(network(input_tensor)[:, 0:1]) + 1.0
            phase = pi * network(input_tensor)[:, 1:2]
            return lambda: concat([tau, phase], 1)

        ctrl = switch_case(idxN, branch_fns={i: get_network_output(i) for i in range(numN)})
...
def main():
    ...
    static_field_estimation(
        args,idxn=int(0), batchsize=1024, max_res=int(10000), learning_rate=1e-3, res='time',
