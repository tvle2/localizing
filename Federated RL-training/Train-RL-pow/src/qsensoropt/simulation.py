...
`
    ...
    def _loop_body(
        ...
        tme = gather(filtered_tensor, index)
        #print(index,tme)  # Output: [3]
        # Apply modulo operation to wrap indices
        #
        # mod_index = math.mod(index, shape(filtered_tensor)[0])
        # # Gather using wrapped indices
        # tme = gather(filtered_tensor, mod_index)

        phase = controls
        tau = expand_dims(tme, axis=-1)  # Expands tau to shape (1,)
        #print('reduce',tau )
        tau = broadcast_to(tau, shape(phase))  # Matches the shape of phase

        controls = concat([tau, phase], 1)
        ...
        #print("new_used_resources",new_used_resources)
        ...
        if not c_flag:
            print(index, c_flag)
            continue_flag = less_equal(
                (pars.max_resources+1)* ones((self.bs, 1),
                                          dtype=pars.prec),
                pars.max_resources * ones((self.bs, 1),
                                          dtype=pars.prec),
                name="continue_flag",
        ...
        #print('true_state---', true_state, '----END')
