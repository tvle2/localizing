...
from tensorflow import cast, ones, \
    gather, concat, reshape, norm, expand_dims, \
        Variable, Tensor,broadcast_to,shape,float32,\
            __version__,config,switch_case,constant,int32,stack
...
def parse_args():
    ...
    parser.add_argument("--trained-models-dir", type=str,
                        default="./nv_center_dc_phaseRange/trained_models")
    parser.add_argument("--data-dir", type=str,
                        default="./indNNi70/data")
...
def static_field_estimation(
    ...
    def control_nn(idxN, input_tensor: Tensor):
        pi = 3.14159265  # more precision
        batch_size = input_tensor.shape[0]

        tau_list = []
        phase_list = []
        #unique_networks = set(idxN.numpy().tolist())
        unique_networks = set([x[0] for x in idxN.numpy().tolist()])

        network_outputs = {}

        # First precompute outputs to avoid redundant network calls
        for net_idx in unique_networks:
            net_out = networks[int(net_idx)](input_tensor)
            network_outputs[net_idx] = net_out

        # Then collect tau and phase
        for i in range(batch_size):
            net_idx = int(idxN[i])
            output = network_outputs[net_idx]
            tau = prefact * abs(output[i, 0:1]) + 1.0
            phase = pi * output[i, 1:2]
            tau_list.append(tau)
            phase_list.append(phase)

        # Stack the lists into tensors
        tau_tensor = stack(tau_list)
        phase_tensor = stack(phase_list)

        ctrl = concat([tau_tensor, phase_tensor], 1)
    ...
    for i in range(numN):
        save_path = "./InDomg20/range"+str(i)+"/trained_models"
        save_path1 = "./InDomg20/range" + "/trained_models"
        file_path = save_path + \
                    "_history_weights/"
        file_name = file_path + str(128)
        networks[i].load_weights(file_name)
        networks[i].save(
            join(save_path1, str(i)),
        )
...
def main():
    ...
    #i = 0
        ...
        static_field_estimation(
            args,idxn=int(i), batchsize=64, max_res=int(10000), learning_rate=1e-3, res='time',
