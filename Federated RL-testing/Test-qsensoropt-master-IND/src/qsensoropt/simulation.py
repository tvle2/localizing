...
`
    ...
    def _loop_body(
        ...
        IDXn = floor(-reduce_max(-particles,axis = 1)+1)
        IDXn = where(IDXn > 9, 9.0, IDXn)
    ...
    def execute(
        ...
        numN = 10
        Nois = [(2*i, 2*(i+1)) for i in range(numN)]#0.5+0.02*i
        ...
        evolution_time = 3.14/ 20 # (2**5) * tau#** jnp.arange(10)#1#3.14/ 20
        n_trials = 100
        n_shots = 70#50#40#60#20#30#50#40,30,20,10#50
        n_grid = 100
        ...
        phis_estimates = ephis_estimate# - jnp.mod(ephis_estimate,0.5)#--Fix --range
        ...
        particles = add(particles,-idxN*2)
        #print(particles,'particles-fix-',n_shots)

        particles = add(particles, phis_estimates-1)
        #particles = add(particles, phis_estimates)#--Fix --range
        print(particles.shape,'2-particles-fix-',n_shots)
