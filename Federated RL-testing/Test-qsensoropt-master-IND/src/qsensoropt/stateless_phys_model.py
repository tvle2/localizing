...
class StatelessPhysicalModel(PhysicalModel):
    ...
    def perform_measurement(
        self, indxN:int,controls: Tensor, parameters: Tensor, meas_step: float,
        rangen: Generator,
    ) -> Tuple[Tensor, Tensor]:
    ...
    def wrapper_perform_measurement(
        self, idxN:int,controls: Tensor, parameters: Tensor, true_state: Tensor,
    ...
    def wrapper_model(
        self,idxN:int, outcomes: Tensor, controls: Tensor, parameters: Tensor,
        state: Tensor, meas_step: float,
        num_systems: int = 1,
    ) -> Tuple[Tensor, Tensor]:
        prob = self.model(idxN,
