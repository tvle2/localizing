...
from tensorflow.math import divide_no_nan, log,reduce_max
...
class StatelessMetrology(StatelessSimulation):
    ...
    def generate_input(
        ...
        for i, par in enumerate(self.pf.phys_model.params):
            ...
            #print(particles[0,:,0],'Particles---par.bounds', maxbounds[:,0],minbounds[0,0])

            mean = 2*divide_no_nan((mean - minbounds),(maxbounds - minbounds))-1
            #print(i, mean, 'par.bounds')
            mean_part = expand_dims(
                mean[:, i], axis=1, name="mean_part",
                #normalize(mean[:, i], par.bounds), axis=1, name="mean_part",
            )
